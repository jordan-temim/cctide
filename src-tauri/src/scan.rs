...
/// The transcript of the interactive session that owns `path`. Sub-agent
/// transcripts live under `…/<sessionId>/subagents/agent-*.jsonl`; they belong
/// to the parent session whose own transcript is `…/<sessionId>.jsonl`, beside
/// that folder. Any other path owns itself.
fn session_root(path: &Path) -> PathBuf {
    let comps: Vec<_> = path.components().collect();
    if let Some(pos) = comps.iter().position(|c| c.as_os_str() == "subagents") {
        if pos >= 1 {
            // comps[..pos] ends at the `<sessionId>` directory; its sibling
            // `<sessionId>.jsonl` is the parent transcript.
            let sid_dir: PathBuf = comps[..pos].iter().collect();
            if let Some(name) = sid_dir
                .file_name()
                .map(|n| n.to_string_lossy().into_owned())
            {
                let mut parent = sid_dir.clone();
                parent.set_file_name(format!("{name}.jsonl"));
                return parent;
            }
        }
    }
...
impl ScanCache {
    ...
    pub fn session_edit_spans(&self, from: i64, now: i64) -> Vec<SessionSpan> {
        // Fold sub-agent transcripts into their parent interactive session, so
        // Outcomes counts one span per session — the same notion the Sessions
        // tab uses (it filters sub-agents out). A sub-agent's quota and edits
        // are attributed to its parent, keeping Outcomes consistent with the
        // gauges instead of inflating the session count with `agent-*.jsonl`.
        struct Acc {
            cwd: Option<String>,
            ts: Vec<i64>,
            weighted: f64,
        ...
        let mut groups: HashMap<PathBuf, Acc> = HashMap::new();
        for (path, file) in &self.files {
            let pts: Vec<&Point> = file
                .points
                .iter()
                .filter(|p| p.ts >= from && p.ts <= now)
                .collect();
            let eds: Vec<EditRec> = file
                .edits
                .iter()
                .filter(|e| e.ts >= from && e.ts <= now)
                .cloned()
                .collect();
            if pts.is_empty() && eds.is_empty() {
                continue;
            }
            let root = session_root(path);
            let is_parent = root == *path;
            let acc = groups.entry(root).or_insert_with(|| Acc {
                cwd: None,
                ts: Vec::new(),
                weighted: 0.0,
                edits: Vec::new(),
            ...
            // Prefer the parent transcript's cwd; sub-agents share it anyway.
            if (is_parent || acc.cwd.is_none()) && file.cwd.is_some() {
                acc.cwd = file.cwd.clone();
            }
        ...
        groups
            .into_values()
            .filter(|a| !a.ts.is_empty())
            .map(|a| SessionSpan {
                cwd: a.cwd,
                first_ts: a.ts.iter().copied().min().unwrap_or(from),
                last_ts: a.ts.iter().copied().max().unwrap_or(now),
                weighted: a.weighted,
                edits: a.edits,
            })
            .collect()
    ...
    pub fn project_cwds_in_window(&self, from: i64, to: i64) -> Vec<String> {
        let mut cwds: std::collections::BTreeSet<String> = Default::default();
        for file in self.files.values() {
            let Some(ref cwd) = file.cwd else { continue };
            if file.points.iter().any(|p| p.ts >= from && p.ts <= to) {
...
mod tests {
    ...
    #[test]
    fn session_root_folds_subagents_into_parent() {
        let parent = PathBuf::from("/root/.claude/projects/enc/5a10.jsonl");
        let sub = PathBuf::from("/root/.claude/projects/enc/5a10/subagents/agent-deadbeef.jsonl");
        assert_eq!(session_root(&parent), parent);
        assert_eq!(session_root(&sub), parent);
        // A plain transcript owns itself.
        let plain = PathBuf::from("/root/.claude/projects/enc/other.jsonl");
        assert_eq!(session_root(&plain), plain);
    }

    #[test]
    fn session_edit_spans_merges_subagents() {
        let mut cache = ScanCache::default();
        // Parent interactive session.
        cache.files.insert(
            PathBuf::from("/p/enc/sess.jsonl"),
            CachedFile {
                points: vec![pt_for(1_000, 10.0)],
                cwd: Some("/home/u/proj".to_string()),
                edits: vec![EditRec {
                    path: "a.rs".to_string(),
                    ts: 1_100,
        ...
        // Two sub-agents it spawned (own transcripts, no cwd recorded here).
        cache.files.insert(
            PathBuf::from("/p/enc/sess/subagents/agent-1.jsonl"),
            CachedFile {
                points: vec![pt_for(2_000, 5.0)],
                edits: vec![EditRec {
                    path: "b.rs".to_string(),
                    ts: 2_100,
                }],
        ...
        cache.files.insert(
            PathBuf::from("/p/enc/sess/subagents/agent-2.jsonl"),
            CachedFile {
                points: vec![pt_for(3_000, 7.0)],
                ..Default::default()
            },
        );

        let spans = cache.session_edit_spans(0, 10_000);
        // One span, not three: the sub-agents fold into the parent session.
        assert_eq!(spans.len(), 1);
        let s = &spans[0];
        // cwd comes from the parent; quota and edits include the sub-agents.
        assert_eq!(s.cwd.as_deref(), Some("/home/u/proj"));
        assert!((s.weighted - 22.0).abs() < 1e-9);
        assert_eq!((s.first_ts, s.last_ts), (1_000, 3_000));
        assert_eq!(s.edits.len(), 2);
    }

    #[test]
    fn session_root_owns_self_for_paths_without_a_parent() {
        // A leading `subagents` component has no `<sessionId>` above it: own self
        // rather than panic or fabricate a bogus parent.
        let weird = PathBuf::from("subagents/agent-x.jsonl");
        assert_eq!(session_root(&weird), weird);
        // Nested sub-agent (defensive — only one level is expected in practice):
        // folds to the directory directly above `subagents`.
        let nested = PathBuf::from("/p/enc/sid/subagents/agent-a/subagents/agent-b.jsonl");
        assert_eq!(session_root(&nested), PathBuf::from("/p/enc/sid.jsonl"));
    }

    #[test]
    fn session_edit_spans_subagent_without_parent_in_cache() {
        // The parent transcript may have no in-window activity (or not be cached
        // at all); the sub-agent must still form one span attributed to the
        // parent session, taking the sub-agent's cwd as a fallback.
        let mut cache = ScanCache::default();
        cache.files.insert(
            PathBuf::from("/p/enc/sess/subagents/agent-1.jsonl"),
            CachedFile {
        ...
        let spans = cache.session_edit_spans(0, 10_000);
        assert_eq!(spans.len(), 1);
        assert_eq!(spans[0].cwd.as_deref(), Some("/home/u/proj"));
        assert!((spans[0].weighted - 5.0).abs() < 1e-9);
    }

    #[test]
    fn session_edit_spans_dedup_point_key_across_subagent() {
        // The same API response can surface in both the parent file and a
        // sub-agent; folding must count its quota once (pt_for keys on ts).
        let mut cache = ScanCache::default();
        cache.files.insert(
            PathBuf::from("/p/enc/sess.jsonl"),
            CachedFile {
        ...
        cache.files.insert(
            PathBuf::from("/p/enc/sess/subagents/agent-1.jsonl"),
            CachedFile {
                // pt_for(1_000, …) shares the parent's key → deduped; the 2_000
                // point is distinct and adds up.
                points: vec![pt_for(1_000, 10.0), pt_for(2_000, 5.0)],
                ..Default::default()
            },
        );
        ...
        let spans = cache.session_edit_spans(0, 10_000);
        assert_eq!(spans.len(), 1);
        assert!((spans[0].weighted - 15.0).abs() < 1e-9);
        assert_eq!((spans[0].first_ts, spans[0].last_ts), (1_000, 2_000));
    }

    #[test]
    fn session_edit_spans_clamps_subagent_to_window() {
        // A sub-agent point outside the window is dropped, like any other; the
        // span keeps only the parent's in-window activity.
        let mut cache = ScanCache::default();
        cache.files.insert(
            PathBuf::from("/p/enc/sess.jsonl"),
            CachedFile {
                points: vec![pt_for(5_000, 10.0)],
                cwd: Some("/proj".to_string()),
                ..Default::default()
            },
        );
        ...
        let spans = cache.session_edit_spans(0, 10_000);
        assert_eq!(spans.len(), 1);
        assert!((spans[0].weighted - 10.0).abs() < 1e-9);
        assert_eq!(spans[0].last_ts, 5_000);
        assert!(spans[0].edits.is_empty());
    }
}
