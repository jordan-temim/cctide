...
//! - edits in commits not (yet) on main                        → `on_branch`
//! - edits whose commits were later reverted                   → `reverted`
//! - no commit carries those edits (still uncommitted)         → `uncommitted`
    ...
    fn temporal_fate(&self, first_ts: i64, last_ts: i64) -> Fate {
        ...
        for c in &self.commits {
            ...
            best = match (best, fate) {
                ...
                (_, Fate::OnBranch) | (Fate::OnBranch, _) => Fate::OnBranch,
...
/// The session's overall fate, by **majority of its edits**. A session usually
/// edits several files; each edit resolves to its own fate (`edit_fate`), and
/// the session takes the fate held by the **most** edits — e.g. 3 shipped + 2
/// uncommitted → Shipped. Ties break toward the better outcome
/// (Shipped > OnBranch > Reverted > Uncommitted), via the worst-to-best array
/// below that `max_by_key` scans keeping the last maximum. A session with no
/// edits inside the repo falls back to the coarse temporal rule instead.
fn classify_span(span: &SessionSpan, repo_root: &str, log: &RepoLog) -> Fate {
    ...
    // resolves toward the better outcome (Shipped > OnBranch > Reverted >
    // Uncommitted).
    [
        Fate::Uncommitted,
        Fate::Reverted,
            ...
            Some(root) => {
                ...
                match log {
                    ...
                    Some(log) => match classify_span(span, &root, log) {
                        ...
                        Fate::OnBranch => "on_branch",
                        Fate::Reverted => "reverted",
                        Fate::Uncommitted => "uncommitted",
                    },
                }
            }
        };
...
mod tests {
    ...
    #[test]
    fn on_branch_when_commit_is_on_unmerged_branch() {
        if !have_git() {
            return;
        }
        let t = base_ts();
        let (_td, repo) = init_repo("on-branch", "main");
        commit_file(&repo, "a.rs", "v1", "init", t);
        git_test(&repo, t, &["checkout", "-q", "-b", "feat"]);
        let s = span(&repo, t + 100, t + 200, vec![edit(&repo, "a.rs", t + 150)]);
        commit_file(&repo, "a.rs", "v2", "wip", t + 300);
        let report = outcome_report(&[s], t, t + 86_400);
        assert_eq!(kind_of(&report, "on_branch"), (100.0, 1));
    }
    ...
    #[test]
    fn uncommitted_when_no_later_commit_touches_file() {
        if !have_git() {
            return;
        }
        let t = base_ts();
        let (_td, repo) = init_repo("uncommitted", "main");
        commit_file(&repo, "a.rs", "v1", "init", t);
        let s = span(&repo, t + 100, t + 200, vec![edit(&repo, "a.rs", t + 150)]);
        let report = outcome_report(&[s], t, t + 86_400);
        assert_eq!(kind_of(&report, "uncommitted"), (100.0, 1));
    }
    ...
    fn edits_outside_repo_are_ignored() {
        ...
        // no commit during the session → uncommitted.
        let s = span(
            &repo,
            t + 100,
            t + 200,
    ...
    fn classify_majority_and_tie_break() {
        ...
        // Majority wins: 2 on-branch vs 1 shipped → OnBranch.
        let span = mk(vec![e("on_branch.rs"), e("on_branch.rs"), e("shipped.rs")]);
        assert_eq!(classify_span(&span, "/repo", &log), Fate::OnBranch);
        // Tie breaks toward the better outcome: 1 shipped vs 1 uncommitted → Shipped.
        let span = mk(vec![e("shipped.rs"), e("never-committed.rs")]);
        assert_eq!(classify_span(&span, "/repo", &log), Fate::Shipped);
    }

    #[test]
    fn merged_branch_flips_on_branch_to_shipped() {
        if !have_git() {
            return;
        }
        let t = base_ts();
        ...
        // Before the merge the session is on a branch…
