...
//!   to estimate the 5h/weekly %. These are **empirical** (from the `lab/`
//!   regression, captured 2026-06-08): output dominates (opus == sonnet,
//!   haiku ~0.1×), `cache_write_1h` ~0.1× output, and `input` /
//!   `cache_write_5m` / `cache_read` ~0. Cache reads are never passed to
//!   `quota_units` and so are always excluded.
//!
